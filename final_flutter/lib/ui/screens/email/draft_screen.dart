...
class DraftScreen extends StatefulWidget {
  ...
  const DraftScreen({super.key, required this.repository, this.fontFamily = 'Roboto', this.fontSize = 16});
...
class _DraftScreenState extends State<DraftScreen> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 100) {
      final total = widget.repository.drafts.length;
  ...
  Widget build(BuildContext context) {
    final emails = widget.repository.drafts.take(_currentMax).toList()
                      ...
                      onTap: _multiSelectMode
                          ...
                          : () {
                              Navigator.of(context).push(PageRouteBuilder(
                                pageBuilder: (context, animation, secondaryAnimation) => ComposeScreen(repository: widget.repository, draft: email),
