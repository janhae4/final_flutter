...
class InboxScreen extends StatefulWidget {
  ...
  const InboxScreen({super.key, required this.repository, required this.notificationEnabled, this.fontFamily = 'Roboto', this.fontSize = 16});
...
class _InboxScreenState extends State<InboxScreen> {
  ...
  void initState() {
    super.initState();
    widget.repository.addListener(_onNewEmail);
  ...
  @override
  void dispose() {
    widget.repository.removeListener(_onNewEmail);
  ...
  void _onNewEmail() {
    setState(() {
      _lastNewEmail = widget.repository.inbox.isNotEmpty ? widget.repository.inbox.first : null;
    });
    if (_lastNewEmail != null && (widget.notificationEnabled)) {
      final email = _lastNewEmail!;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Email mới từ ${email.sender}\n${email.subject} (${_formatTime(email.time)})'),
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 100) {
      final total = widget.repository.inbox.length;
  ...
  Widget build(BuildContext context) {
    final emails = widget.repository.inbox.take(_currentMax).toList()
    ...
    return Scaffold(
      ...
      floatingActionButton: _multiSelectMode
          ...
          : FloatingActionButton(
              onPressed: _mockReceiveEmail,
              tooltip: 'Nhận email mới (mock)',
              child: Icon(Icons.mark_email_unread),
            ),
    );
  }

  void _mockReceiveEmail() {
    final now = DateTime.now();
    widget.repository.receiveEmail(Email(
      id: now.microsecondsSinceEpoch.toString(),
      sender: 'new.sender@example.com',
      to: ['me@example.com'],
      cc: [],
      bcc: [],
      subject: 'Email mới lúc ${_formatTime(now)}',
      content: 'Đây là nội dung email mới nhận được.',
      time: now,
    ));
  }
